    ...
    parseaprs = function(packet: string, options?: any) {
        ...
        // Object
            ...
            if($paclen >= 31) {
                ...
                this._object_to_decimal($options, $body, $srccallsign, $rethash);
            }
            */
        // NMEA data
        } else if($packettype == '$') {
 ...
 *
FAP.mice_mbits_to_message = function($bits) {
 ...
 *
FAP.distance = function($lon0, $lat0, $lon1, $lat1) {
    // decimal to radian
 ...
 *
FAP.direction = function($lon0, $lat0, $lon1, $lat1) {
    $lon0 = this.deg2rad($lon0);
    $lon1 = this.deg2rad($lon1);
    $lat0 = this.deg2rad($lat0);
    $lat1 = this.deg2rad($lat1);
 ...
 *
FAP.count_digihops = function($header) {
    let tmp;
 ...
 *
FAP._get_posresolution = function(dec) {
 ...
 *
FAP._nmea_getlatlon = function($value, $sign, $rethash) {
    let tmp;
 ...
 *
FAP._get_symbol_fromdst = function($dstcallsign) {
    /*
    let $table;
    let $code;
 ...
 *
FAP._nmea_to_decimal = function($options, $body, $srccallsign, $dstcallsign, $rethash) {
    let tmp;
    /*
    if ($debug > 1) {
 ...
 * as well as comment
 *
FAP._comments_to_decimal = function($rest, $srccallsign, $rethash) {
    let tmprest;
 ...
 * Parse an object
 *
FAP._object_to_decimal = function($options, $packet, $srccallsign, $rethash) {
    let tmp;

    // Minimum length for an object is 31 characters
    // (or 46 characters for non-compressed)
    if($packet.length < 31) {
        this.addError($rethash, 'obj_short');
        return 0;
    }
    ...
    // to zero.
    $rethash['timestamp'] = this._parse_timestamp($options, $timestamp);

    if($rethash['timestamp'] == 0) {
        this.addWarning($rethash, 'timestamp_inv_obj');
    }
    ...
    if(/^[\/\\A-Za-j]$/.test($locationchar)) {
        // compressed
        $retval = this._compressed_to_decimal($packet.substr(18, 13), $srccallsign, $rethash);
        $locationoffset = 31; // now points to APRS data extension/comment
    } else if(/^\d$/i.test($locationchar)) {
        // normal
        $retval = this._normalpos_to_decimal($packet.substr(18), $srccallsign, $rethash);
        $locationoffset = 37; // now points to APRS data extension/comment
    } else {
        // error
        this.addError($rethash, 'obj_dec_err');
        return 0;
    }
    ...
    if($rethash['symbolcode'] != '_') {
        ...
        this._wx_parse($packet.substr($locationoffset), $rethash);
    }
 ...
 *
FAP._capabilities_parse = function($packet, $srccallsign, $rethash) {
    /*
    # Remove CRs, LFs and trailing spaces
    $packet =~ tr/\r\n//d;
    $packet =~ s/\s+$//;
...
FAP._comment_telemetry = function($rethash, $rest) {
 ...
 * Parse an item
 *
FAP._item_to_decimal = function($packet, $srccallsign, $rethash) {
    let tmp;
 ...
 *
FAP._normalpos_to_decimal = function($packet, $srccallsign, $rethash) {
    // Check the length
    if($packet.length < 19) {
 ...
 *
FAP._mice_to_decimal = function($packet, $dstcallsign, $srccallsign, $rethash, $options) {
    let tmp;
    $rethash['format'] = 'mice';
 ...
 *
FAP._compressed_to_decimal = function($packet, $srccallsign, $rethash) {
 ...
 *
FAP._dao_parse = function($daocandidate, $srccallsign, $rethash) {
 ...
 *
FAP._dx_parse = function($sourcecall, $info, $rethash) {
    /*
 ...
 *
FAP._wx_parse = function($s, $rethash) {
 ...
 *
FAP._wx_parse_peet_packet = function($s, $sourcecall, $rethash) {
    // warn "\$ULTW: $s\n";
 ...
 *
FAP._wx_parse_peet_logging = function($s, $sourcecall, $rethash) {
    // warn "\!!: $s\n";
    // 0000000001FF000427C70002CCD30001026E003A050F00040000
    let $w = {};
    let $t;
 ...
 *
FAP._telemetry_parse = function($s, $rh) {
    let $t = {};
 ...
 *
FAP._kiss_checkcallsign = function($callsign) {
 ...
 * or undef on error.
 *
FAP.kiss_to_tnc2 = function($kissframe) {
    /*
    let $asciiframe = '';
    let $dstcallsign = '';
    let $callsigntmp = '';
 ...
 *
FAP.tnc2_to_kiss = function($gotframe) {
 ...
 *
FAP.aprs_duplicate_parts = function($packet) {
    /*
...
*
FAP.make_object = function($name, $tstamp, $lat, $lon, $symbols, $speed, $course
        , $altitude, $alive, $usecompression, $posambiguity, $comment) {
 ...
 *
FAP.make_timestamp = function($tstamp, $tformat) {
    if($tstamp == 0) {
 ...
 * parameters should be changed to hash with named parameters.
 *
FAP.make_position = function($lat, $lon, $speed, $course, $altitude, $symbols, $usecompression, $posambiguity) {
